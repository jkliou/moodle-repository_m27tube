...
class repository_m27tube extends repository {
    /** @var int maximum number of thumbs per page */
    const M27TUBE_THUMBS_PER_PAGE = 27;
    ...
    public function set_option($options = array()) {
        if (!empty($options['apikey'])) {
            set_config('apikey', trim($options['apikey']), 'm27tube');
        }
    ...
    public function get_option($config = '') {
        if ($config === 'apikey') {
            return trim(get_config('m27tube', 'apikey'));
        } else {
            $options['apikey'] = trim(get_config('m27tube', 'apikey'));
        }
    ...
    public function search($search_text, $page = 0) {
        global $SESSION;
        $sort = optional_param('m27tube_sort', '', PARAM_TEXT);
        $sess_keyword = 'm27tube_'.$this->id.'_keyword';
        $sess_sort = 'm27tube_'.$this->id.'_sort';
        ...
        $start = ($ret['page'] - 1) * self::M27TUBE_THUMBS_PER_PAGE + 1;
        $max = self::M27TUBE_THUMBS_PER_PAGE;
    ...
    /**
     * Private method to get m27tube search results
    ...
    private function _get_collection($keyword, $start, $max, $sort) {
        ...
        $sesspagetoken = 'm27utube_'.$this->id.'_nextpagetoken';
        $pagetoken = '';
        ...
        $error = null;
        try {
            $this->init_m27tube_service(); // About to use the service, ensure it's loaded.
            ...
            throw new moodle_exception('apierror', 'repository_m27tube', '', $error);
        }
    ...
    public function print_login($ajax = true) {
        ...
        $search = new stdClass();
        $search->type = 'text';
        $search->id   = 'm27tube_search';
        $search->name = 's';
        $search->label = get_string('search', 'repository_m27tube').': ';
        $sort = new stdClass();
        $sort->type = 'select';
        $sort->options = array(
            (object)array(
                'value' => 'relevance',
                'label' => get_string('sortrelevance', 'repository_m27tube')
            ),
            (object)array(
                'value' => 'date',
                'label' => get_string('sortpublished', 'repository_m27tube')
            ),
            (object)array(
                'value' => 'rating',
                'label' => get_string('sortrating', 'repository_m27tube')
            ),
            (object)array(
                'value' => 'viewCount',
                'label' => get_string('sortviewcount', 'repository_m27tube')
            )
        );
        $sort->id = 'm27tube_sort';
        $sort->name = 'm27tube_sort';
        $sort->label = get_string('sortby', 'repository_m27tube').': ';
    ...
    /**
     * file types supported by m27tube plugin
     * @return array
     */
    ...
    public static function type_config_form($mform, $classname = 'repository') {
        ...
        $apikey = get_config('m27tube', 'apikey');
        if (empty($apikey)) {
            $apikey = '';
        }

        $mform->addElement('text', 'apikey', get_string('apikey', 'repository_m27tube'), array('value' => $apikey, 'size' => '40'));
        $mform->setType('apikey', PARAM_RAW_TRIMMED);
        $mform->addRule('apikey', get_string('required'), 'required', null, 'client');

        $mform->addElement('static', null, '',  get_string('information', 'repository_m27tube'));
    }
